...
public class AdapterDao<T> implements InterfazDao<T> {
    ...
    public LinkedList<T> listAll() { // Convierte el String con formato Json en un Array de objetos
        ...
        try {
            String data = readFile(); // Invoca el método readFile() para leer el archivo JSON y devolverlo en formato
                                      // String
            T[] matrix = (T[]) g.fromJson(data, java.lang.reflect.Array.newInstance(clazz, 0).getClass()); // Deserializa
                                                                                                           // el String
                                                                                                           // JSON en un
                                                                                                           // Array de
                                                                                                           // objetos
    ...
    private String readFile() throws Exception { // Lee el archivo JSON y lo convierte en un String
        File file = new File(filePath + clazz.getSimpleName() + ".json"); // Crea una instancia de File con la ruta del
                                                                          // archivo JSON
